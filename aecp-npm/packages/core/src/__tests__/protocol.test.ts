...
describe('AECP Protocol', () => {
  ...
  describe('Embedding', () => {
    test('embeds single text', async () => {
      const agent = new AECP({ embedder: new MockEmbedder(384) });
  ...
  describe('Calibration', () => {
    test('calibrates two agents successfully', async () => {
      const agent1 = new AECP({ embedder: new MockEmbedder(384, 'model-1') });
      const agent2 = new AECP({ embedder: new MockEmbedder(512, 'model-2') });
    ...
    test('stores transfer matrices bidirectionally', async () => {
      ...
      const agent2 = new AECP({ embedder: new MockEmbedder(512) });
  ...
  describe('Edge Cases', () => {
    test('handles same-dimension calibration', async () => {
      const agent1 = new AECP({ embedder: new MockEmbedder(384) });
      const agent2 = new AECP({ embedder: new MockEmbedder(384) });
    ...
    test('handles large dimension difference', async () => {
      const agent1 = new AECP({ embedder: new MockEmbedder(128) });
      const agent2 = new AECP({ embedder: new MockEmbedder(1024) });
