...
export class AECPError extends Error {
...
export class CalibrationError extends AECPError {
...
export class TransferError extends AECPError {
  constructor(message: string, recoverable = true, context: Record<string, any> = {}) {
...
export class NegotiationError extends AECPError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, true, context);
...
export class AdapterError extends AECPError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, true, context);
...
export class CircuitOpenError extends AECPError {
...
export interface AECPResult {
  method: 'aecp';
  data: any;
  language: null;
}

export type GracefulResult = TextFallbackResult | AECPResult;
...
export class GracefulDegradation {
  ...
  async send(
    message: string,
    aecpFn?: () => any | Promise<any>,
    fallbackMetadata?: Record<string, any>,
  ): Promise<GracefulResult> {
    if (aecpFn) {
      try {
        const result = await aecpFn();
        this.aecpCount++;
        return { method: 'aecp', data: result, language: null };
      } catch {
  ...
  getStats(): Record<string, any> {
    const total = this.aecpCount + this.fallbackCount;
    return {
      aecpCount: this.aecpCount,
      fallbackCount: this.fallbackCount,
      total,
      aecpPercentage: total > 0 ? (this.aecpCount / total) * 100 : 0,
