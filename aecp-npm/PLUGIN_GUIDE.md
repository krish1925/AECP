...
AECP provides a plugin system that lets you create custom embedding provider adapters without modifying the core library.
...
```python
from aecp import register_adapter, adapter, AECP
...
```typescript
import { registerAdapter, adapter, AECP, EmbeddingProvider } from '@aecp/core';
...
// Use it
import { createAdapter } from '@aecp/core';
const adapter = createAdapter('my-provider', { apiKey: '...' });
const agent = new AECP({ embedder: adapter });
```
...
```typescript
import { adapter, EmbeddingProvider } from '@aecp/core';
...
// Use it
import { AECP, createAdapter } from '@aecp/core';
...
setup(
    ...
    install_requires=["aecp>=1.0.0"],
)
```

Users install with: `pip install my-aecp-adapter`
...
// package.json
{
  "name": "@your-org/aecp-adapter-my-provider",
  "peerDependencies": {
    "@aecp/core": "^1.0.0"
  }
}
...
Users install with: `npm install @your-org/aecp-adapter-my-provider`
...
7. **Version dependencies** - Specify compatible AECP versions in your package
