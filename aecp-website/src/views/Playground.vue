  ...
  <div>
    <section class="hero">
      <div class="container">
        ...
        <p class="hero-subtitle">Experiment with AECP embedding transfers in real-time</p>
      </div>
    </section>
          ...
          <div class="playground-panel">
            ...
            <div class="transfer-buttons">
              ...
              >
                ...
                {{ transferring ? 'Transferring...' : 'Transfer with AECP' }}
              </button>
              <button 
                @click="transferText" 
            ...
            <div v-if="result" class="result-section">
              ...
              <div class="results-comparison">
                <div class="result-card aecp-result">
                  <h4><i class="fas fa-network-wired"></i> AECP Transfer</h4>
                  <div class="metric">
                    <span class="metric-label">Similarity:</span>
                    <span class="metric-value" :class="getQualityClass(result.aecp.similarity)">
                      {{ (result.aecp.similarity * 100).toFixed(1) }}%
                    </span>
                  </div>
                  <div class="metric">
                    <span class="metric-label">Latency:</span>
                    <span class="metric-value">{{ result.aecp.latency }}ms</span>
                  </div>
                  <div class="quality-bar">
                    <div 
                      class="quality-fill" 
                      :style="{ width: (result.aecp.similarity * 100) + '%' }"
                    ></div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
          ...
          datasets: [
            {
              label: 'AECP',
              data: [
                this.result.aecp.similarity * 100,
                1000 / parseFloat(this.result.aecp.latency)
              ],
    ...
    drawVisualization() {
      ...
      // AECP transfer
      const aecpX = centerX + scale * this.result.aecp.similarity
      const aecpY = centerY - 30
      ...
      ctx.fillStyle = '#0066cc'
      ctx.beginPath()
      ctx.arc(aecpX, aecpY, 10, 0, 2 * Math.PI)
      ctx.fill()
